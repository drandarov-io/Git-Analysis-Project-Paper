...
```{r change-count-by-modification-kind, fig.width=7.5, fig.height=7.5, warning=FALSE}
# # Load data
...
```{r change-count-by-modification-kind-and-author, fig.width=10, fig.height=10, warning=FALSE}
# # Load data
...
```{r change-count-by-month, fig.width=10, fig.height=10, warning=FALSE}
# # Load data
...
```{r change-counts-of-falk-sippach, fig.width=10, fig.height=10, warning=FALSE}
# # Load data
...
```{r change-count-by-file-type, fig.width=10, fig.height=15, warning=FALSE}
# # Load data
...
```{r change-counts-of-author-by-file-type, fig.width=10, fig.height=10, warning=FALSE}
# # Load data
...
```{r ownership-of-repository-of-author-by-file-types, fig.width=10, fig.height=10, warning=FALSE}
# # Load data
csv <- read.table(file.path("../../result/dukecon/wip/Change-Count-by-File-Type-by-Author.csv"), sep=",", header=TRUE)
...
grid.arrange(plot1, plot2, nrow=2)
```
Man sieht, dass obwohl wir vorher gesehen hatten, dass Alexander Schwartz mehr an Java als an Groovy gearbeitet hat, er im Vergleich zu Falk Sippach relativ gesehen doch weniger zum Java-Anteil des Projekts beigesteuert hat als vorher zu vermuten war.
...
```{r ownership-of-authors-by-most-relevant-packages, fig.width=10, fig.height=15, warning=FALSE}
# # Load data
...
```{r ownership-of-authors-by-most-relevant-files, fig.width=10, fig.height=15, warning=FALSE}
# # Load data
...
Ein zwar nicht allzu genaues, aber interessantes Maß sind die Vorkommen der Wörter in den Commit-Messages.
...
```{r commit-massage-by-most-used-words, fig.width=10, fig.height=5, warning=FALSE}
# # Load data
...
Man sieht, dass hauptsächlich die Wörter "added", "merge" und "dukecon" (der Name des Repositories) vorkommen. Das ist normal für ein Projekt allgemein und die Vorkommen von "merge" deuten lediglich auf die Integrierung vieler Open-Source-Kontributionen hin. "Fix" und "Fixed" kommt zwar öfter vor, ist aber nicht auffällig häufig.
...
```{r commit-message-by-most-used-words-file-type, fig.width=10, fig.height=5, warning=FALSE}
# # Load data
...
Das gleiche lässt sich auch mit der Aufteilung nach Autoren machen.
...
```{r commit-message-by-most-used-words-author, fig.width=10, fig.height=5, warning=FALSE}
# # Load data
