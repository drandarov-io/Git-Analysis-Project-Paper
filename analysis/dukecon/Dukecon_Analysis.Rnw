...
ggplot(data, aes(x=reorder(Path, Pct), y=Pct, fill=Author)) +
  ...
  coord_flip() +
  scale_y_continuous(limits=c(0,100), expand=c(0, 0)) +
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.30, size=7.5), axis.title=element_text(size=7.5,face="bold"), title=element_text(size=7.5,face="bold")) +
  labs(x="", y="ChangeCount", title="Ownership of Packages")
@
